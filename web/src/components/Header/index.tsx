import { DarkModeToggleSwitch } from '@/components/DarkModeToggleSwitch';
import { useDarkMode } from '@/hooks/useDarkMode';

export function Header() {
  const { componentMounted, theme } = useDarkMode();
  const surfaceStyle = 'surface-secondary-hover';
  const menuItemStyle = `${surfaceStyle} w-16 h-4 rounded-lg shrink-0`;

  return (
    <header className="surface-secondary">
      <div className="max-w-7xl m-auto h-[72px] p-4 flex items-center">
        <div className="flex flex-1 items-center">
          <div className={`${surfaceStyle} w-40 h-10 rounded-lg shrink-0`} />
          <ul className="flex gap-4 items-center p-4 m-auto">
            <li className={menuItemStyle} />
            <li className={menuItemStyle} />
            <li className={menuItemStyle} />
            <li className={menuItemStyle} />
          </ul>
        </div>
        <div className="flex gap-4 items-center ml-auto">
          {componentMounted && <DarkModeToggleSwitch isChecked={theme === 'dark'} />}
          <div className={`${surfaceStyle} w-8 h-8 rounded-lg shrink-0`} />
          <div className={`${surfaceStyle} w-12 h-12 rounded-full shrink-0`} />
        </div>
      </div>
    </header>
  );
}
